...
for slice = sliceList
    ...
    for row = 1:size(tensors_block,1)
        ...
        for col = 1:size(tensors_block,2)
           ...
           if blockmask(row,col,slice)
            ...
            [Xdata, Ydata, Zdata, cdata] = ellipsoid_OD(DT_mat,mysphere,scalefactor);
 ...
 %%     archive and compress files to .tar.gz
    ...
    if nFiles == size(tensors_block,1)*size(tensors_block,2)
